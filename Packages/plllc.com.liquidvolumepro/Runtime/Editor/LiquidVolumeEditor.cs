...
namespace LiquidVolumeFX
{
    ...
    public class LiquidVolumeEditor : Editor
    {
        ...
        bool GetShaderOptionState(string option)
        {
            ...
            string dataPathCut = Application.dataPath.Substring(0, Application.dataPath.Length - datapath.Length);
            packagePath = Path.Combine(dataPathCut, "Packages");

            string[] res = Directory.GetFiles(packagePath, LV_SHADER_OPTIONS_FILE, SearchOption.AllDirectories);
            string path = null;
            path = res[0];
